...
import { isNumber, pairMap, reduce, join, reverse, map, matrix } from "./lib";
 ...
 */
const withMatrix = (cols, indexChanger) => ([num, index, queue]) =>
  ({
    num,
    index,
    queue,
    ...matrix(indexChanger ? indexChanger(num.length, index) : index)
  }(4, reverseIndex));
 ...
 */
const translate = ({ digits, units, quots }) => data =>
 ...
 *  @return {string}        结果中文
 */
const handleInteger = dict =>
  compose(
    map(compose(withMatrix, checkSection, translate(dict))),
    reverse,
 ...
 *  @return {string}         字符串
 */
const handleDecimal = dict => numbers =>
  numbers === undefined
    ? ""
...
const parserNumbers = numbers => {
  const isMinus =
    numbers.indexOf("-") !== 0 ? false : +(numbers = numbers.slice(1)) || true;
...
const numToChinese = (numbers, dict) =>
  compose(
    parserNumbers,
    pairMap([handleMinus, handleInteger, handleDecimal])(dict),
    join
  )(numbers);

const to = (numbers, dictCode = "chinese") =>
  numToChinese(numbers, Dict[dictCode]);
